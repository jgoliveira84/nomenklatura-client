...
class Dataset(NKObject):
    ...
    def get_entity(self, id):
        """ get an entity from the dataset
            ...
            if id is not specified - raises a ValueError
        """

        if id is None:
            raise ValueError("Need to give an ID.")
        code, val = self._get('/entities/%s' % str(id))
    ...
    def ensure_entity(self, name, data={}):
        ...
        try:
            return self.lookup(name, readonly=True)
        except NKException:
            return self.add_entity(name, data=data)
